...
```
NOTE: Given the rate limit of 100 daily API calls to the streaming availability API service, server-side caching has been implemented with Redis to avoid repeating API calls for the same movie. Therefore, the application supports providing streaming availability information for 100 unique movies daily.
```
